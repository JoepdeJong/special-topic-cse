    ...
    for mutator in mutators:
        for i in range(number_of_mutations):
            ...
            for s in range(len(mutation_steps)):
                ...
                graph = mutator(graph_copy, step - k)
                k += step
                ...
                # Save the measures
                output.append([n, k, r, mutator.__name__, i, s, df, h, h_norm, spectral_scaling, spectral_gap, root_is_leader])
            del graph_copy
...
for n in n_nodes:
    for r in reciprocal_threshold:
        ...
        results = Parallel(n_jobs=num_cores)(delayed(mutate)(n, r, row) for index, row in data_n_p.iterrows())
...
data.to_csv('mutations2_m' + str(m) + '_k0' + str(k0) + '_initial_nodes' + str(initial_nodes)+'.csv', index=False)